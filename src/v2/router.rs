...
macro_rules! router {
    { $($path: literal => $endpoint: ident),* $(,)?} => {
        $crate::v2::Router::from([
            $(
                $crate::v2::router::Route::new(
                    $path.to_string(),
                    std::sync::Arc::new($endpoint(std::sync::Mutex::new($crate::v2::state::State::default())))
                ),
            ),*
        ])
    };
    [ $($endpoint: ident),* $(,)?] => {
        $crate::v2::Router::from([
            $(
                $crate::v2::router::Route::from_endpoint(
                    std::sync::Arc::new(
                        $endpoint( std::sync::Mutex::new($crate::v2::state::State::default()) )
                    )
                ),
            ),*
        ])
